...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_analysis_cache_analysis_date'), 'analysis_cache', ['analysis_date'], unique=False)
    op.create_index(op.f('ix_analysis_cache_user_id'), 'analysis_cache', ['user_id'], unique=False)
    op.create_table('goals',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('user_id', sa.Uuid(), nullable=False),
    sa.Column('goal_type', sqlmodel.sql.sqltypes.AutoString(length=50), nullable=False),
    ...
    sa.Column('schedule_expression', sqlmodel.sql.sqltypes.AutoString(length=100), nullable=False),
    ...
    sa.Column('experience_level', sqlmodel.sql.sqltypes.AutoString(length=50), nullable=True),
    ...
    sa.Column('measurements', postgresql.JSON(astext_type=sa.Text()), nullable=True),
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_index(op.f('ix_workout_sessions_completed_date'), 'workout_sessions', ['completed_date'], unique=False)
    ...
    op.create_index(op.f('ix_exercise_logs_workout_session_id'), 'exercise_logs', ['workout_session_id'], unique=False)
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index(op.f('ix_exercise_logs_workout_session_id'), table_name='exercise_logs')
    op.drop_table('exercise_logs')
    op.drop_index(op.f('ix_workout_sessions_user_id'), table_name='workout_sessions')
    op.drop_index(op.f('ix_workout_sessions_completed_date'), table_name='workout_sessions')
